...
import { BookOpen, Edit3, Download, Upload, Volume2, CheckCircle, Clock, X, Globe, AlignLeft, Sparkles, StopCircle, BookA, Languages, Undo2, Search, ArrowDown, GraduationCap, Library, Save, Trash2, BookMarked, ChevronLeft, ChevronRight, Plus, Eye, EyeOff, MessageCircle, Send, Lightbulb, Minimize, Maximize, ChevronFirst, ChevronLast, LayoutList, Map, Info, Target, Shuffle, Layers } from 'lucide-react';
...
const RTL_LANGS = ['ar', 'he', 'fa', 'ps', 'sd', 'ur', 'ug', 'yi'];
...
export default function App() {
  ...
  const isRTL = currentLang && RTL_LANGS.includes(currentLang.code);

  const [isLibraryVisible, setIsLibraryVisible] = useState(true);
  
  const [isRightPanelOpen, setIsRightPanelOpen] = useState(true);
  const [rightPanelTab, setRightPanelTab] = useState('vocab'); 
  ...
  const [planChapterMode, setPlanChapterMode] = useState('focus'); 
  const [customPlanTopic, setCustomPlanTopic] = useState("");
  ...
  const [audioVoice, setAudioVoice] = useState('Aoede'); 
  const audioVoiceRef = useRef('Aoede');

  // Activity Center State
  const [isChatOpen, setIsChatOpen] = useState(false);
  const [activityMode, setActivityMode] = useState('free_chat'); // 'free_chat', 'translation', 'cloze', 'scramble', 'variation_lab'
  ...
  const handleInputTextChange = (e) => {
    ...
    setActiveChapters(prev => {
      const next = [...prev];
      if (next.length === 0) {
        next.push({ text: newText, level: storyLevel, meta: storyMeta, topic: storyTopic, chapterTitle: "Chapter 1", grammarFocus: activePlanChapterId ? studyPlan.find(p => p.id === activePlanChapterId)?.title : "" });
      } else {
  ...
  const chatTokensArray = useMemo(() => {
    return chatMessages.map(msg => {
      const textToTokenize = activityMode === 'variation_lab' && msg.role === 'model' ? (msg.currentSentence || msg.text) : msg.text;
      return tokenizeText(textToTokenize, currentLang?.code);
    });
  }, [chatMessages, currentLang?.code, activityMode]);
      ...
      parts: [{ 
        text: msg.role === 'user' 
          ...
          : JSON.stringify({ 
              ...
              hiddenTarget: msg.hiddenTarget, 
              message: msg.message,
              currentSentence: msg.currentSentence,
              task: msg.task,
              hint: msg.hint,
        ...
        if (lang) {
          ...
          const migratedLibrary = rawLibrary.map(item => {
             ...
             return {
                ...
                title: item.title,
                date: item.date,
                chapters: [{ text: item.text, level: item.level || 'A1', meta: item.meta || "", topic: item.topic || "", chapterTitle: item.chapterTitle || "Chapter 1", grammarFocus: item.grammarFocus || "" }]
             };
          });
          ...
          if (savedText) {
            ...
            setActiveChapters([{ text: savedText, level: "A1", meta: "", topic: "", chapterTitle: "Chapter 1", grammarFocus: "" }]);
            setIsReading(true);
          } else {
            setInputText("");
            setIsReading(false);
  ...
  const handleLanguageChange = (e) => {
    ...
    if (newText) {
      setInputText(newText);
      setActiveChapters([{ text: newText, level: "A1", meta: "", topic: "", chapterTitle: "Chapter 1", grammarFocus: "" }]);
      setIsReading(true);
    } else {
      setInputText("");
      setIsReading(false);
  ...
  const handleGenerateStudyPlan = async () => {
    ...
    let sysInstruction = "";
    if (customPlanTopic.trim()) {
      sysInstruction = `You are an expert curriculum designer. Create a comprehensive, sequenced study plan blueprint about "${customPlanTopic.trim()}" for a student learning ${currentLang.name}.
      
      CRITICAL RULES:
      1. Break the specific topic down logically into 10-20 chapters.
      2. Ensure it provides a highly structured way to systematically explore this specific subject in ${currentLang.name}.
      3. Return ONLY a valid JSON array of objects with exactly this structure, NO markdown formatting:
      [
        { "id": "1", "title": "Chapter Topic Name", "description": "Brief explanation of what this chapter covers" }
      ]`;
    } else {
      sysInstruction = `You are an expert linguist and language curriculum designer. Create a comprehensive, grammatically-sequenced study plan blueprint for learning ${currentLang.name}.
  ...
  const handleExplainPlanChapter = async (e, chapter) => {
    ...
    setInfoPopup({ title: `Explanation: ${chapter.title}`, isLoading: true });

    let prompt = `Explain the concept or topic of "${chapter.title}" (${chapter.description}) for a student learning ${currentLang.name}. Provide a clear, highly structured English explanation, detailing its context, construction, usage, or narrative. Provide several practical examples or key sentences in ${currentLang.name} along with their translations.`;
    
    if (currentLang.code === 'el') {
      prompt += ` CRITICAL: You MUST explain and provide examples using ONLY Standard Modern Greek (Κοινή Νεοελληνική). Do NOT use Ancient Greek, Koine, or Katharevousa.`;
    }
    if (currentLang.code === 'ar') {
      prompt += ` CRITICAL: Frame the explanation and examples in the Palestinian dialect (اللَّهْجَة الْفِلَسْطِينِيَّة).`;
    }
    ...
    setInfoPopup({ 
      title: `Explanation: ${chapter.title}`, 
     ...
     try {
       ...
       if (jsonMatch) {
         ...
         return {
           ...
           currentSentence: parsed.currentSentence || parsed.message || parsed.text || "...",
           task: parsed.task || parsed.message || "Modify the sentence.",
           hint: parsed.hint || "",
           isCorrect: parsed.isCorrect !== undefined ? parsed.isCorrect : null
         };
       }
     } catch (e) {
       console.error("Failed to parse Teacher JSON response", e);
     }
     return { feedback: "", hiddenTarget: "", message: rawText, currentSentence: rawText, task: "Modify the sentence.", hint: "", isCorrect: null };
  };

  const getActivitySystemInstruction = (mode) => {
    const activePlan = studyPlan.find(p => p.id === activePlanChapterId);
    
    let sysInstruction = `Target Language: ${currentLang.name}. User Level: CEFR ${storyLevel}.
    Curriculum (User's Meta Instructions): ${storyMeta.trim() || 'General'}.
    Context Topic: ${storyTopic.trim() || 'Everyday topics'}.`;

    if (activePlan) {
      if (planChapterMode === 'focus') {
         sysInstruction += `\nCRITICAL LANGUAGE FOCUS: You MUST specifically focus the exercise on practicing this concept/vocabulary: "${activePlan.title}" (${activePlan.description}).`;
      } else {
         sysInstruction += `\nCRITICAL TOPIC FOCUS: You MUST specifically center the exercise around this topic: "${activePlan.title}" (${activePlan.description}).`;
      }
    }
    
    if (currentLang.code === 'el') sysInstruction += `\nCRITICAL RULE: You MUST use and accept ONLY Standard Modern Greek (Κοινή Νεοελληνική). Do NOT use Ancient Greek or Katharevousa.`;
    if (currentLang.code === 'ar') sysInstruction += `\nCRITICAL RULE: You MUST frame the language and expected answers in the Palestinian dialect (اللَّهْجَة الْفِلَسْطِينِيَّة).`;
    
    sysInstruction += `\n\nRULES FOR THIS TURN:\n`;
    
    if (mode === 'translation') {
       sysInstruction += `1. Evaluate the user's previous translation (if any). Put your evaluation and corrections in the 'feedback' field (STRICTLY IN ENGLISH).
       2. Generate a new, natural sentence IN ENGLISH for the user to translate into ${currentLang.name}. Put this English sentence in the 'message' field. Make it relevant to the Context Topic or Focus.
       3. Put the correct/expected ${currentLang.name} translation in the 'hiddenTarget' field.`;
    } else if (mode === 'cloze') {
       sysInstruction += `1. Evaluate the user's previous answer (if any) in 'feedback' (STRICTLY IN ENGLISH).
       2. Generate a new sentence in ${currentLang.name}, but replace 1 or 2 critical target words with '___'. Put this incomplete sentence in 'message' (IN ${currentLang.name.toUpperCase()}).
       3. Put the exact missing word(s) in 'hiddenTarget' (IN ${currentLang.name.toUpperCase()}).`;
    } else if (mode === 'scramble') {
       sysInstruction += `1. Evaluate the user's previous answer (if any) in 'feedback' (STRICTLY IN ENGLISH).
       2. Generate a correct sentence in ${currentLang.name}, then entirely SCRAMBLE the word order. Put the scrambled words (separated by slashes / ) in 'message' (IN ${currentLang.name.toUpperCase()}), asking the user to unscramble it into a grammatically correct sentence.
       3. Put the correctly ordered ${currentLang.name} sentence in 'hiddenTarget' (IN ${currentLang.name.toUpperCase()}).`;
    } else if (mode === 'variation_lab') {
       sysInstruction += `1. Evaluate the user's previous variation (if any). Determine if they correctly applied the requested linguistic transformation and maintained grammatical correctness. Set "isCorrect" to boolean true or false. Explain why in 'feedback' (STRICTLY IN ENGLISH).
       2. If the user was INCORRECT, keep the previous 'currentSentence' and repeat the 'task' and 'hint' (perhaps adding more detail to the hint).
       3. If the user was CORRECT (or if this is the very first turn), their new valid sentence becomes the new 'currentSentence'. If this is the first turn, invent a highly natural, COMPLETELY RANDOM AND DIVERSE Seed Sentence in ${currentLang.name} (do not use the same verbs/topics repeatedly).
       4. Generate a NEW 'task' IN ENGLISH. Tell the user the CONCEPTUAL goal (e.g., "Change the sentence to say 'We want to find a table'"). Do NOT provide the target language words or grammar formulas in the 'task'. Throw them into the deep water!
       5. Generate a 'hint' IN ENGLISH. This is the EXPLICIT linguistic mechanics to use (e.g., "Use the verb 'θέλω' (want), the particle 'να', and the 1st person plural subjunctive"). Use elements from the Linguistic Variation Taxonomy here.
       6. Put the exact expected target sentence in 'hiddenTarget' (IN ${currentLang.name.toUpperCase()}).`;
    } else {
       sysInstruction += `1. Secretly decide on a specific "Hidden Target" (a word or phrase in ${currentLang.name}) you want the user to type in their NEXT reply.
       2. Evaluate the user's previous message. Correct grammar gently in the 'feedback' field (STRICTLY IN ENGLISH).
       3. Ask a natural conversational question in 'message' (STRICTLY IN ${currentLang.name.toUpperCase()}) designed to make the user naturally reply using your Hidden Target. NEVER use the Hidden Target word in your own prompt.
       4. Put the target word in 'hiddenTarget'.`;
    }
    
    if (mode === 'variation_lab') {
       sysInstruction += `\n\n7. You MUST ALWAYS return ONLY a valid JSON object with EXACTLY these keys (do NOT add "thought" or any extra keys):
       {
         "feedback": "Your evaluation/feedback STRICTLY IN ENGLISH.",
         "isCorrect": true or false,
         "currentSentence": "The base sentence in ${currentLang.name} to be modified",
         "task": "The conceptual instruction IN ENGLISH (e.g. 'Make it a question asking if they want a table.'). Do NOT spoon-feed the vocabulary here.",
         "hint": "The explicit linguistic mechanics to use (e.g. 'Use the verb X in the subjunctive...').",
         "hiddenTarget": "The exact expected result in ${currentLang.name}"
       }`;
    } else {
       sysInstruction += `\n\n5. You MUST ALWAYS return ONLY a valid JSON object with EXACTLY these three keys (do NOT add "thought" or any other extra keys):
       {
         "feedback": "Your evaluation/feedback STRICTLY IN ENGLISH. Be encouraging but clear.",
         "hiddenTarget": "The exact word/phrase/sentence expected next",
         "message": "Your actual prompt/question/exercise"
       }`;
    }
       
    if (currentLang.code === 'zh-CN' || currentLang.code === 'zh-TW') {
       sysInstruction += `\n6. CRITICAL: For Chinese, include Hanzi AND Pinyin in your outputs.`;
    }
    
    return sysInstruction;
  };

  const handleOpenChat = async (modeOverride = activityMode) => {
    if (!currentLang) return;
    setIsChatOpen(true);
    
    if (chatMessages.length === 0 || modeOverride !== activityMode) {
      setActivityMode(modeOverride);
      setChatMessages([]);
      setIsChatLoading(true);
      
      const sysInstruction = getActivitySystemInstruction(modeOverride);
      const prompt = `Start the session. Output ONLY valid JSON matching the system instructions. Random Seed: ${Math.floor(Math.random() * 100000)}. CRITICAL: Make sure the initial seed sentence uses a completely random topic, verb, and scenario to ensure it is different every single time.`;
      ...
      if (rawResponse) {
        ...
        setChatMessages([{ 
          role: 'model', 
          text: modeOverride === 'variation_lab' ? parsed.currentSentence : parsed.message, 
          feedback: parsed.feedback, 
          hiddenTarget: parsed.hiddenTarget,
        ...
        setChatMessages([{ role: 'model', text: "Hello! Let's start. Please refresh or try again." }]);
      }
  ...
  const handleSendChatMessage = async () => {
    ...
    const sysInstruction = getActivitySystemInstruction(activityMode);
    ...
    if (rawResponse) {
      ...
      setChatMessages([...newMessages, { 
        role: 'model', 
        text: activityMode === 'variation_lab' ? parsed.currentSentence : parsed.message,
        feedback: parsed.feedback,
        hiddenTarget: parsed.hiddenTarget,
        currentSentence: parsed.currentSentence,
        task: parsed.task,
  ...
  const handleSaveChatAsBook = () => {
    ...
    const chatText = chatMessages.map(msg => {
      if (activityMode === 'variation_lab') {
         if (msg.role === 'model') {
            let t = `[Base Sentence]: ${msg.currentSentence || msg.text}\n[Task]: ${msg.task || msg.text}`;
            if (msg.feedback) t = `[Feedback: ${msg.feedback}]\n` + t;
            return t;
         } else {
            return `[My Variation]: ${msg.text}`;
         }
      } else {
    ...
    let modeName = "Conversation";
    if (activityMode === 'translation') modeName = "Translation Drills";
    if (activityMode === 'cloze') modeName = "Cloze Drills";
    if (activityMode === 'scramble') modeName = "Sentence Scramble";
    if (activityMode === 'variation_lab') modeName = "Variation Lab";

    let resolvedTitle = `${modeName}: ${storyTopic.trim() || 'Session'}`;
    const activePlan = studyPlan.find(p => p.id === activePlanChapterId);
    ...
    const newBook = {
      ...
      chapters: [{
        ...
        meta: storyMeta,
        topic: storyTopic,
        chapterTitle: "Activity Transcript",
        grammarFocus: activePlan ? activePlan.title : ""
      }]
    };
  ...
  const saveCurrentStory = () => {
    ...
    const activePlan = studyPlan.find(p => p.id === activePlanChapterId);
    let currentChaps = activeChapters.length > 0 ? activeChapters : [{ 
      text: inputText, 
      level: storyLevel, 
      meta: storyMeta, 
      topic: storyTopic, 
  ...
  const handleGenerateStory = async () => {
    ...
    if (activePlan) {
      if (planChapterMode === 'focus') {
        prompt += `\n\nCRITICAL LANGUAGE FOCUS: This text MUST be designed specifically to practice this linguistic/thematic concept: "${activePlan.title}" (${activePlan.description}). Embed natural examples of this structure/vocabulary heavily into the text.`;
      } else {
        prompt += `\n\nCRITICAL NARRATIVE PLOT: You MUST write the story specifically to narrate and explain this topic/event: "${activePlan.title}" (${activePlan.description}). Make this the central plot of the text.`;
      }
    }
  ...
  const handleExplainGrammar = async () => {
    ...
    if (currentLang.code === 'el') {
      prompt += ` CRITICAL: Analyze this using ONLY Standard Modern Greek (Κοινή Νεοελληνική). Do NOT use Ancient Greek.`;
    }
    if (currentLang.code === 'zh-CN' || currentLang.code === 'zh-TW') {
      prompt += ` CRITICAL RULE: Because the language is Chinese, you MUST seamlessly include its Simplified Hanzi (普通话), Traditional Hanzi (普通話), and Pinyin (Pǔtōnghuà) natively in your explanation text.`;
    }
  ...
  return (
    ...
    >
      ...
      <div className="max-w-[1800px] mx-auto w-full px-4 py-6 flex flex-col xl:flex-row gap-6 items-start">
          ...
          {currentLang ? (
            isChatOpen ? (
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col h-[600px] xl:h-[calc(100vh-120px)] overflow-hidden" onClick={(e) => e.stopPropagation()}>
                 <div className="bg-indigo-600 text-white px-5 py-4 flex justify-between items-center shrink-0">
                   <div>
                     <h2 className="font-bold flex items-center gap-2 text-lg">
                       <Target className="w-5 h-5"/> Activity Center: {currentLang?.name}
                     </h2>
                   </div>
                   <div className="flex items-center gap-2">
                     <button onClick={handleSaveChatAsBook} className="flex items-center gap-1.5 bg-emerald-500 hover:bg-emerald-400 px-3 py-1.5 rounded-lg text-sm font-bold transition-colors" title="Save Session as Book">
                     ...
                     <div className="h-6 w-px bg-indigo-400 mx-2"></div>
                     <button onClick={() => handleOpenChat(activityMode)} className="p-2 bg-indigo-700 hover:bg-indigo-800 rounded-full transition-colors" title="Restart Activity">
                       <Undo2 className="w-4 h-4"/>
                     </button>
                     <button onClick={() => setIsChatOpen(false)} className="p-2 text-indigo-200 hover:text-white transition-colors" title="Close Activity Center">
                       <X className="w-6 h-6"/>
                     </button>
                   </div>
                 </div>
                 ...
                 <div className="bg-white border-b border-slate-200 px-4 py-2 flex gap-2 overflow-x-auto shrink-0 shadow-sm z-10 scrollbar-hide">
                    {[
                      { id: 'free_chat', label: 'Free Chat', icon: MessageCircle },
                      { id: 'translation', label: 'Translation Drills', icon: Languages },
                      { id: 'cloze', label: 'Cloze (Fill-in-the-blank)', icon: Edit3 },
                      { id: 'scramble', label: 'Sentence Scramble', icon: Shuffle },
                      { id: 'variation_lab', label: 'Sentence Variation Lab', icon: Layers }
                    ].map(mode => (
                      <button
                        key={mode.id}
                        onClick={() => {
                          if (activityMode !== mode.id) {
                            handleOpenChat(mode.id);
                          }
                        }}
                 ...
                 <div className={`flex-1 overflow-y-auto p-4 sm:p-6 bg-slate-50 flex flex-col gap-4 relative ${activityMode === 'variation_lab' ? 'before:absolute before:inset-0 before:mx-auto before:w-1 before:bg-indigo-100' : ''}`} ref={chatScrollRef} onClick={() => { setSelectedWords([]); setInfoPopup(null); }}>
                    {chatMessages.length === 0 && !isChatLoading && (
                       <div className="m-auto text-slate-400 text-center relative z-10 bg-slate-50 p-4 rounded-xl">
                         {activityMode === 'variation_lab' ? <Layers className="w-12 h-12 mx-auto mb-3 opacity-20" /> : <Target className="w-12 h-12 mx-auto mb-3 opacity-20" />}
                         <p>Ready to start! Select an activity above or say hello.</p>
                       </div>
                    )}
                    {chatMessages.map((msg, idx) => {
                       if (activityMode === 'variation_lab') {
                           return (
                               <div key={idx} className="w-full flex flex-col items-center relative z-10">
                                   {msg.role === 'model' ? (
                                       <div className="w-full max-w-2xl bg-white rounded-2xl border border-indigo-100 shadow-sm p-5 md:p-8 relative mt-4">
                                           {idx > 0 && (
                                              <div className="absolute -top-4 left-1/2 -translate-x-1/2 text-indigo-300 bg-slate-50 rounded-full">
                                                 <ArrowDown className="w-6 h-6"/>
                                              </div>
                                           )}
                                           
                                           {msg.feedback && (
                                               <div className={`text-sm mb-6 p-3 rounded-lg border ${msg.isCorrect ? 'bg-emerald-50 text-emerald-800 border-emerald-200' : 'bg-red-50 text-red-800 border-red-200'}`} dir="ltr">
                                                   {msg.isCorrect ? <CheckCircle className="w-4 h-4 inline mr-1.5 -mt-0.5"/> : <X className="w-4 h-4 inline mr-1.5 -mt-0.5"/>}
                                                   {msg.feedback}
                                               </div>
                                           )}
                                           
                                           <div className="text-center mb-6">
                                               <span className="text-xs font-extrabold uppercase tracking-widest text-indigo-400">Base Sentence</span>
                                               <div className="text-2xl font-bold text-indigo-900 mt-2 flex items-center justify-center gap-3 leading-relaxed">
                                                  <div dir={isRTL ? "rtl" : "ltr"}>
                                                      {renderInteractiveTokens(chatTokensArray[idx], `chat-${idx}`)}
                                                  </div>
                                                  <button onClick={() => playWordPhraseAudio(msg.currentSentence || msg.message)} className="text-indigo-400 hover:text-indigo-600 bg-indigo-50 p-1.5 rounded-full"><Volume2 className="w-5 h-5"/></button>
                                               </div>
                                           </div>
                                           
                                           <div className="bg-indigo-50 rounded-xl p-4 md:p-5 border border-indigo-200 text-center relative shadow-inner">
                                               <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-indigo-200 text-indigo-800 px-3 py-0.5 rounded-full text-xs font-extrabold uppercase tracking-wider border border-indigo-300">Next Task</span>
                                               <div className="text-lg font-medium text-slate-800 mt-2" dir="ltr">{msg.task || msg.message}</div>
                                           </div>
                                           
                                           {msg.hint && (
                                               <div className="mt-5 flex flex-col items-center">
                                                   <button onClick={() => toggleChatHint(idx)} className={`text-xs font-extrabold uppercase tracking-wider flex items-center gap-1 transition-colors px-3 py-1 rounded-full border ${msg.isHintOpen ? 'bg-amber-100 text-amber-600 border-amber-200' : 'bg-white text-slate-400 hover:text-amber-500 hover:border-amber-200 border-slate-200 shadow-sm'}`}>
                                                       <Lightbulb className="w-4 h-4"/> {msg.isHintOpen ? "Hide Hint" : "Show Hint"}
                                                   </button>
                                                   {msg.isHintOpen && (
                                                       <div className="mt-3 bg-amber-50 text-amber-800 border border-amber-200 px-4 py-2 rounded-xl text-sm font-medium shadow-sm leading-relaxed" dir="ltr">
                                                           {msg.hint}
                                                       </div>
                                                   )}
                                               </div>
                                       ...
                                       <div className="w-full max-w-xl bg-blue-600 text-white rounded-2xl p-5 shadow-md text-center mt-6 relative">
                                           <div className="absolute -top-6 left-1/2 -translate-x-1/2 text-blue-400 bg-slate-50 rounded-full">
                                              <ArrowDown className="w-6 h-6"/>
                                           </div>
                                           <span className="text-xs font-extrabold uppercase tracking-widest text-blue-200 mb-2 block">Your Variation</span>
                                           <div className="text-xl font-bold leading-relaxed" dir={isRTL ? "rtl" : "ltr"}>{msg.text}</div>
                                       </div>
                                   )}
                               </div>
                           );
                           ...
                           const enforceRTL = msg.role === 'model' && activityMode === 'translation' ? false : isRTL;
                           return (
                             <div key={idx} className={`flex flex-col max-w-[85%] relative z-10 ${msg.role === 'user' ? 'self-end items-end' : 'self-start items-start'}`}>
                                <div className={`px-4 py-3 text-[1.05rem] leading-relaxed shadow-sm relative group ${
                                   ...
                                   {msg.role === 'model' && msg.feedback && (
                                     <div className="text-sm text-indigo-600 mb-2 italic border-b border-indigo-100 pb-2" dir="ltr">
                                        {msg.feedback}
                                     </div>
                                   )}
                                   
                                   <div dir={enforceRTL ? "rtl" : "ltr"}>
                                   ...
                                   {msg.role === 'model' && msg.isHintOpen && msg.hiddenTarget && (
                                     <div className="mt-3 text-sm bg-amber-50 text-amber-800 border border-amber-200 p-2 rounded flex items-center gap-2">
                                       ...
                                       <span><strong>Hint/Target:</strong> <em dir={isRTL ? "rtl" : "ltr"}>{msg.hiddenTarget}</em></span>
                                     </div>
                                   )}
                                </div>
                             </div>
                    ...
                    {isChatLoading && (
                       <div className="self-start items-start flex gap-2 text-slate-400 px-4 py-3 relative z-10 bg-white rounded-2xl border border-slate-200 shadow-sm">
                 ...
                 <div className="p-4 bg-white border-t border-slate-200 shrink-0 z-20 shadow-[0_-4px_6px_-1px_rgb(0,0,0,0.05)]">
                    <div className="flex gap-2 relative">
                      <input 
                        ...
                        placeholder={activityMode === 'translation' ? `Type your translation in ${currentLang?.name}...` : `Type your response in ${currentLang?.name}...`}
                      ...
                      <button 
                        ...
                        className="absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 text-white rounded-lg transition-colors shadow-sm"
                      >
            ...
            <>
              {/* Top Toolbar (View Toggles, Undo, Edit Mode) */}
              <div className="flex justify-end mb-4">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <button
                    onClick={(e) => toggleRightPanel('plan')}
                    className={`flex items-center justify-center w-10 h-10 rounded-lg shadow-sm border transition-colors ${isRightPanelOpen && rightPanelTab === 'plan' ? 'bg-blue-50 border-blue-200 text-blue-700' : 'bg-white border-slate-300 text-slate-700 hover:bg-slate-100'}`}
                    title="Toggle Study Plan"
                  >
                    <Map className="w-5 h-5" />
                  </button>
                  <button
                    onClick={(e) => toggleRightPanel('vocab')}
                    className={`flex items-center justify-center w-10 h-10 rounded-lg shadow-sm border transition-colors ${isRightPanelOpen && rightPanelTab === 'vocab' ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-white border-slate-300 text-slate-700 hover:bg-slate-100'}`}
                    title="Toggle Vocab Viewer"
                  >
                  ...
                  <button 
                    ...
                    className="flex items-center gap-2 bg-slate-800 border border-slate-700 text-white hover:bg-slate-900 px-4 py-2 rounded-lg shadow-sm font-medium transition-colors ml-2"
                  >
                    {isReading ? (
              ...
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 md:p-8 min-h-[400px] flex flex-col relative" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="flex flex-col gap-2">
                      <div className="bg-blue-100 border border-blue-300 rounded-lg px-3 py-2 flex items-center justify-between shadow-sm">
                         <div className="flex items-center gap-2">
                           ...
                           <span className="text-sm font-semibold text-blue-800">Plan: {activePlanChapter.title}</span>
                         </div>
                         <button 
                      ...
                      <div className="flex items-center gap-1 bg-white border border-blue-200 rounded-lg p-1 w-full max-w-[280px]">
                        <button
                          onClick={() => setPlanChapterMode('focus')}
                          className={`flex-1 py-1 px-3 text-xs font-bold rounded-md transition-colors ${planChapterMode === 'focus' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-50'}`}
                          title="Generate a story that practices the language concepts of this chapter"
                        >
                          Language Focus
                        </button>
                        <button
                          onClick={() => setPlanChapterMode('plot')}
                          className={`flex-1 py-1 px-3 text-xs font-bold rounded-md transition-colors ${planChapterMode === 'plot' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-50'}`}
                          title="Generate a story that narrates the actual topic/plot of this chapter"
                        >
                          Story Plot
                        </button>
                      </div>
                    ...
                    <div className="flex flex-col gap-3 w-full md:w-auto">
                      ...
                      <div className="flex flex-col gap-2 w-full">
                        ...
                        <button 
                          ...
                          title="Open the Activity Center to practice this topic"
                        >
                   ...
                   <div className="mb-6 flex flex-col gap-1 border-b border-slate-200 pb-4">
                       {isEditingBookTitle ? (
                         ...
                         <div className="group relative flex items-center gap-3 mb-2">
                           <div className="text-3xl font-extrabold text-slate-900 leading-normal flex-1" dir={isRTL ? "rtl" : "ltr"}>
                       ...
                       {isEditingChapterTitle ? (
                         ...
                         <div className="group relative flex items-center gap-3">
                           <div className="text-xl font-bold text-slate-500 leading-normal flex-1" dir={isRTL ? "rtl" : "ltr"}>
                       ...
                       {(activeChapters[activeChapterIndex]?.topic || activeChapters[activeChapterIndex]?.meta || activeChapters[activeChapterIndex]?.grammarFocus) && (
                ...
                {/* Story Action Toolbar (Play, Translate, Prep, Search) */}
                {isReading && inputText && (
                  <div className="mb-6 flex flex-col xl:flex-row flex-wrap items-start xl:items-center justify-between gap-4 bg-slate-50 p-3 rounded-xl border border-slate-200 shadow-sm">
                    <div className="flex flex-wrap items-center gap-2">
                      {isPlayingFullStory ? (
                        <button 
                    ...
                    {!isSentenceMode && (
                      <div 
                        className="flex items-center gap-2 bg-white px-3 py-2 rounded-lg border border-slate-300 shadow-sm w-full xl:w-auto"
                        onClick={(e) => e.stopPropagation()}
                      >
                        <Search className="w-4 h-4 text-slate-400" />
                        <input 
                          ...
                          className="w-full xl:w-32 focus:outline-none text-sm text-slate-700"
                        />
                  ...
                  >
                    {isGeneratingStory ? (
                       ...
                       <div className="flex flex-col items-center justify-center py-10 md:py-20 min-h-[300px] transition-all">
                          <div className="text-3xl md:text-4xl leading-relaxed md:leading-loose text-center max-w-4xl text-slate-900 transition-all duration-300" dir={isRTL ? "rtl" : "ltr"}>
                       ...
                       <div className="text-xl md:text-2xl leading-relaxed md:leading-loose" dir={isRTL ? "rtl" : "ltr"}>
                  ...
                  <div className="mt-8 p-5 bg-indigo-50/50 border border-indigo-100 rounded-xl flex flex-col gap-3">
                      ...
                      <div className="flex flex-col gap-2">
                        <div className="bg-indigo-100 border border-indigo-300 rounded-lg px-3 py-2 flex items-center justify-between shadow-sm">
                           <div className="flex items-center gap-2">
                             ...
                             <span className="text-sm font-semibold text-indigo-900">Plan: {activePlanChapter.title}</span>
                           </div>
                           <button 
                             onClick={() => setActivePlanChapterId(null)} 
                        ...
                        <div className="flex items-center gap-1 bg-white border border-indigo-200 rounded-lg p-1 w-full max-w-[280px]">
                          <button
                            onClick={() => setPlanChapterMode('focus')}
                            className={`flex-1 py-1 px-3 text-xs font-bold rounded-md transition-colors ${planChapterMode === 'focus' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-50'}`}
                            title="Generate a story that practices the language concepts of this chapter"
                          >
                            Language Focus
                          </button>
                          <button
                            onClick={() => setPlanChapterMode('plot')}
                            className={`flex-1 py-1 px-3 text-xs font-bold rounded-md transition-colors ${planChapterMode === 'plot' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-50'}`}
                            title="Generate a story that narrates the actual topic/plot of this chapter"
                          >
                            Story Plot
                          </button>
                    ...
                    <div className="flex flex-col md:flex-row gap-3">
                      <textarea 
                        ...
                        rows={2}
                        disabled={isGeneratingStory}
                        dir={isRTL && nextChapterTopic ? "rtl" : "ltr"}
                      />
                      <textarea 
        ...
        {/* RIGHT SIDEBAR: TABBED PANEL (STUDY PLAN & VOCAB VIEWER) */}
        <aside className={`w-full xl:w-72 2xl:w-80 flex-shrink-0 bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col h-[500px] xl:h-[calc(100vh-120px)] xl:sticky xl:top-24 ${!isRightPanelOpen ? 'hidden xl:flex xl:w-0 xl:opacity-0 xl:overflow-hidden xl:border-none xl:p-0 transition-all' : 'transition-all'}`} onClick={e => e.stopPropagation()}>
          
          {/* Tab Switcher Header */}
          <div className="flex border-b border-slate-200 bg-slate-50 rounded-t-xl shrink-0">
             <button
               onClick={() => setRightPanelTab('vocab')}
               className={`flex-1 py-3 text-sm font-bold flex justify-center items-center gap-2 border-b-2 transition-colors ${rightPanelTab === 'vocab' ? 'border-emerald-500 text-emerald-700 bg-white' : 'border-transparent text-slate-500 hover:text-slate-700 hover:bg-slate-100'}`}
             >
               <BookMarked className="w-4 h-4" /> Vocab
             </button>
             <button
               onClick={() => setRightPanelTab('plan')}
               className={`flex-1 py-3 text-sm font-bold flex justify-center items-center gap-2 border-b-2 transition-colors ${rightPanelTab === 'plan' ? 'border-blue-500 text-blue-700 bg-white' : 'border-transparent text-slate-500 hover:text-slate-700 hover:bg-slate-100'}`}
             >
               <Map className="w-4 h-4" /> Study Plan
             </button>
          </div>
            ...
            <div className="flex flex-col h-full overflow-hidden">
              <div className="p-3 border-b border-slate-200 flex flex-col gap-3 shrink-0">
                <div className="flex justify-between gap-2">
                  <div className="flex items-center gap-2 bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200 flex-1">
                    <Search className="w-4 h-4 text-slate-400" />
                    <input 
                      type="text"
                      value={vocabSearch}
                  ...
                  <button 
                    onClick={handleUndo}
                    disabled={vocabHistory.length === 0}
                    className="text-slate-500 hover:text-emerald-700 bg-slate-100 px-2 rounded-lg transition-colors disabled:opacity-50"
                ...
                <div className="flex justify-between items-center gap-2">
                  ...
                  <button 
                    ...
                    disabled={!currentLang}
                    className="flex-1 flex items-center justify-center gap-1.5 px-2 py-1.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-600 rounded-lg text-xs font-medium transition-colors disabled:opacity-50"
                  ...
                  <button 
                    ...
                    disabled={!currentLang || Object.keys(vocab).length === 0}
                    className="flex-1 flex items-center justify-center gap-1.5 px-2 py-1.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-600 rounded-lg text-xs font-medium transition-colors disabled:opacity-50"
                        ...
                        >
                          <div className="flex justify-between items-start">
                            <div className="relative group inline-block max-w-fit">
                              <span className="font-bold text-slate-800 text-base cursor-pointer inline-block" dir={isRTL ? "rtl" : "ltr"}>{wordKey}</span>
                              {romanization && (
          ...
          {rightPanelTab === 'plan' && (
            <div className="flex flex-col h-full overflow-hidden">
              <div className="p-3 border-b border-slate-200 shrink-0 flex flex-col gap-3">
                 <input
                   type="text"
                   value={customPlanTopic}
                   onChange={(e) => setCustomPlanTopic(e.target.value)}
                   placeholder="Custom topic (optional)..."
                   className="w-full px-3 py-2 rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-400 text-sm text-slate-700"
                   disabled={isGeneratingPlan || !currentLang}
                 />
                 <button 
                  ...
                  >
                    ...
                    {customPlanTopic.trim() ? "Generate Custom Plan" : "Generate Curriculum"}
                  </button>
                           ...
                           <div key={i} className="bg-white border border-purple-200 p-3 rounded-lg shadow-sm flex flex-col justify-center gap-1 hover:border-purple-400 transition-colors">
                             <div className="flex justify-between items-start">
                               <div className="group relative inline-block">
                                 <span className="font-bold text-slate-800 text-lg cursor-help border-b border-dashed border-slate-300 pb-0.5" dir={isRTL ? "rtl" : "ltr"}>{word.term}</span>
                         ...
                         {ppsData.structures?.map((struct, i) => (
                           <div key={i} className="bg-white border border-purple-200 p-4 rounded-lg shadow-sm group relative hover:border-purple-400 transition-colors">
                             <div className="flex justify-between items-start mb-2">
                               <div className="group relative inline-block">
                                 <span className="font-bold text-indigo-700 text-lg cursor-help" dir={isRTL ? "rtl" : "ltr"}>{struct.template}</span>
              ...
              <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-4 flex-1">
                  ...
                  <div className="overflow-hidden">
                    <div className="flex items-center gap-2">
                      <h2 className="text-2xl font-bold text-slate-800 tracking-wide truncate" title={displayPhrase} dir={isRTL ? "rtl" : "ltr"}>
                        {displayPhrase}
                      </h2>
                      <button 
