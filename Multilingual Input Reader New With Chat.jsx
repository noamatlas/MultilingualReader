...
import { BookOpen, Edit3, Download, Upload, Volume2, CheckCircle, Clock, X, Globe, AlignLeft, Sparkles, StopCircle, BookA, Languages, Undo2, Search, ArrowDown, GraduationCap, Library, Save, Trash2, BookMarked, ChevronLeft, ChevronRight, Plus, Eye, EyeOff, MessageCircle, Send } from 'lucide-react';
...
const TRANSLITERATE_LANGS = ['am', 'hy', 'bn', 'zh-CN', 'zh-TW', 'ka', 'gu', 'hi', 'ja', 'kn', 'kk', 'km', 'ko', 'ky', 'lo', 'ml', 'mr', 'mn', 'my', 'ne', 'or', 'ps', 'fa', 'pa', 'sd', 'si', 'tg', 'ta', 'tt', 'te', 'th', 'ur', 'ug'];
...
export default function App() {
  ...
  // Library & Chapter Hierarchy State
  ...
  // Full Chapter Translation State
  ...
  // Universal Popup state (Etymology & Grammar)
  ...
  useEffect(() => {
    ...
    let isMounted = true;
    setHasTTS(false); 
    const testAudio = new Audio(`https://translate.google.com/translate_tts?ie=UTF-8&client=tw-ob&tl=${currentLang.code}&q=a`);
    testAudio.oncanplaythrough = () => { if (isMounted) setHasTTS(true); };
    testAudio.onerror = () => { if (isMounted) setHasTTS(false); };
  ...
  // Gemini API Caller (Native TTS - Chapter Level Only)
  const fetchGeminiAudio = async (text, langCode, langName) => {
    const apiKey = "";
  ...
  const getAudioUrl = async (text, lang) => {
    const cacheKey = `${lang.code}_${text}`;
    ...
    const promise = fetchGeminiAudio(text, lang.code, lang.name).then(url => {
  ...
  useEffect(() => {
    let isMounted = true;
    if (isReading && inputText && currentLang && hasTTS) {
       const chunks = splitIntoChunks(inputText, 600);
       const prefetchAllAudio = async () => {
         for (const chunk of chunks) {
           if (!isMounted) break;
           await getAudioUrl(chunk, currentLang);
         }
       };
       prefetchAllAudio();
    }
    return () => { isMounted = false; };
  }, [isReading, inputText, currentLang, hasTTS]);

  useEffect(() => {
    if (!currentLang || !TRANSLITERATE_LANGS.includes(currentLang.code)) return;

    const allTextWords = [
      ...bodyTokens.filter(t => t.isWordLike).map(t => cleanWord(t.text)),
      ...titleTokens.filter(t => t.isWordLike).map(t => cleanWord(t.text)),
      ...chapterTitleTokens.filter(t => t.isWordLike).map(t => cleanWord(t.text))
    ];
    const vocabWords = Object.keys(vocab);
    const uniqueWords = [...new Set([...allTextWords, ...vocabWords])];
    const missingWords = uniqueWords.filter(w => !romanizationCacheRef.current[w]);

    if (missingWords.length === 0) return;

    let isMounted = true;
    const fetchRomanizations = async () => {
       const batchSize = 30; 
       for (let i = 0; i < missingWords.length; i += batchSize) {
           if (!isMounted) break;
           const batch = missingWords.slice(i, i + batchSize);
           const prompt = `You are a helpful linguistic assistant. Provide the Latin transliteration (e.g., Pinyin with tone marks for Chinese, Romaji for Japanese, standard Latin transliteration for others) for the following ${currentLang.name} words.
           CRITICAL: Return ONLY a valid JSON object where keys are the exact original words provided, and values are their transliterations. Do NOT wrap in markdown blocks, just raw JSON.
           ...
           if (result && isMounted) {
               try {
                   ...
                   setRomanizationCache(prev => {
                       const next = { ...prev, ...parsed };
                       safeStorage.setItem(`rom_${currentLang.code}`, JSON.stringify(next));
                       return next;
                   });
               } catch (e) {
                   console.error("Failed to parse transliteration batch", e, result);
               }
           }
    ...
    fetchRomanizations();
    return () => { isMounted = false; };
  }, [bodyTokens, titleTokens, chapterTitleTokens, vocab, isReading, currentLang]);
  ...
  }, [vocab, translationCache, romanizationCache, etymologyCache, inputText, library, currentLang, isInitialized]);
  ...
  const handleOpenChat = async () => {
    ...
    if (chatMessages.length === 0) {
      ...
      const prompt = `Generate a very short opening greeting (1-2 sentences) in ${currentLang.name} to start a friendly roleplay conversation. 
      Target Level: CEFR ${storyLevel}. 
      Topic: ${storyTopic.trim() || 'General everyday conversation'}. 
      Meta instructions: ${storyMeta.trim() || 'None'}.
      End your greeting with a question to invite the user to respond.
      ${currentLang.code === 'zh-CN' || currentLang.code === 'zh-TW' ? 'CRITICAL: You MUST include Simplified Hanzi, Traditional Hanzi, and Pinyin in your text (e.g. 苹果 / 蘋果 | píngguǒ).' : ''}`;
  ...
  const handleSendChatMessage = async () => {
    ...
    let sysInstruction = `You are a friendly and helpful language conversation partner roleplaying with the user. 
    Target Language: ${currentLang.name}.
    User's Level: CEFR ${storyLevel}.
    Conversation Topic: ${storyTopic.trim() || 'General everyday conversation'}.
    User's Meta Instructions: ${storyMeta.trim() || 'None'}.
    
    CRITICAL RULES:
    1. Respond EXCLUSIVELY in ${currentLang.name}.
    2. Adapt your vocabulary and grammar strictly to CEFR level ${storyLevel}.
    3. Keep responses conversational and relatively concise (1-4 sentences).
    4. Correct the user gently if they make a major grammar mistake in their previous message, but prioritize keeping the roleplay flowing.
    5. End with a natural question or prompt to keep the user engaged.`;

    if (currentLang.code === 'zh-CN' || currentLang.code === 'zh-TW') {
      sysInstruction += `\n6. CRITICAL: For Chinese, you MUST include Simplified Hanzi, Traditional Hanzi, and Pinyin natively in your text. Format strictly as: 苹果 / 蘋果 (píngguǒ).`;
    }

    const responseText = await callGeminiChatAPI(newMessages, sysInstruction);
  ...
  const playWordPhraseAudio = async (text) => {
    ...
    const url = `https://translate.google.com/translate_tts?ie=UTF-8&client=tw-ob&tl=${currentLang.code}&q=${encodeURIComponent(text)}`;
  ...
  const playNextInQueue = async () => {
    ...
    const url = await getAudioUrl(text, currentLang);
    ...
    if (audioQueueRef.current.length > 0) {
      getAudioUrl(audioQueueRef.current[0], currentLang);
    }
  ...
  const playFullStory = () => {
    if (!inputText || !currentLang) return;
    stopAudio();
    isPlayingSequenceRef.current = true;
    audioQueueRef.current = splitIntoChunks(inputText, 600);
    if (audioQueueRef.current.length > 0) {
  ...
  const handleGenerateStory = async () => {
    ...
    let prompt = "";
    if (storyLevel === "A0") {
      prompt = `Write an absolute beginner comprehensible input story (about 20-40 words) at a 'Pre-A1' (Zero) level ${topicPhrase} in ${currentLang.name}. Use extreme simplicity: very short sentences, basic present-tense grammar, and a strictly limited total vocabulary of only 4 to 10 unique words repeated naturally. Return ONLY the ${currentLang.name} text, no English translation, no intro, and no markdown formatting.`;
    } else {
      prompt = `Write a short, engaging comprehensible input story (about 100-150 words) at CEFR level ${storyLevel} ${topicPhrase} in ${currentLang.name}. Ensure the grammar and vocabulary perfectly match the requested level. Return ONLY the ${currentLang.name} text, no English translation, no intro, and no markdown formatting.`;
    }
  ...
  const handleGenerateNextChapter = async () => {
    ...
    if (nextChapterLevel === "A0") {
      prompt += `Keep this new chapter at an absolute beginner 'Pre-A1' (Zero) level (about 20-40 words, very simple sentences, and a strictly limited total vocabulary of only 4 to 10 unique words repeated naturally). `;
    } else {
      prompt += `Make this new chapter about 100-150 words at CEFR level ${nextChapterLevel}. Ensure the grammar and vocabulary perfectly match this level. `;
    }
  ...
  const toggleFullStoryTranslation = async () => {
    ...
    const cacheKey = `full_${inputText.trim()}`;
    ...
    if (trans) {
      ...
      setFullStoryTranslation("Chapter translation currently unavailable.");
    }
  ...
  useEffect(() => {
    ...
    const currentTokens = source === 'title' ? titleTokens : source === 'chapter' ? chapterTitleTokens : bodyTokens;
    ...
    while (end < currentTokens.length - 1 && !/[.;!?\n]/.test(currentTokens[end].text)) end++;
  ...
  }, [selectedWords, currentLang, bodyTokens, titleTokens, chapterTitleTokens]);
  ...
  const renderInteractiveTokens = (tokens, source) => {
    ...
    return tokens.map((tokenObj) => {
      ...
      if (!isWordLike) {
        ...
        return <span key={`${source}-${index}`} className={`whitespace-pre-wrap text-slate-700 ${isSearchMatch ? 'bg-yellow-200' : ''}`}>{token}</span>;
      }
      ...
      let colorClasses = "text-slate-800 cursor-pointer transition-colors px-0.5 rounded";
      ...
      if (showHighlights) {
        if (status === 'known') {
          ...
          colorClasses += " hover:bg-blue-100";
        }
      } else {
        colorClasses += " hover:bg-slate-200"; 
      }
      ...
      return (
        <span 
          ...
          className={`relative group inline-block ${colorClasses} ${isSelected ? 'ring-2 ring-blue-500' : ''} ${isSearchMatch ? 'bg-yellow-200 ring-2 ring-yellow-400' : ''}`}
        >
          {romanization && (
  ...
  const filteredLibrary = library.filter(book => 
    ...
    book.chapters.some(c => c.text.toLowerCase().includes(librarySearch.toLowerCase()))
  );

  const vocabList = Object.entries(vocab)
    .filter(([word]) => word.toLowerCase().includes(vocabSearch.toLowerCase()))
    .sort((a, b) => {
  ...
  return (
    ...
    >
      {/* Header */}
      <header className="bg-blue-700 text-white shadow-md px-4 py-4 md:px-8 flex flex-col md:flex-row items-center justify-between gap-4 sticky top-0 z-40">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto justify-end">
          ...
          <button 
            ...
            disabled={!currentLang}
            className="flex items-center gap-2 bg-blue-800 hover:bg-blue-900 px-3 py-2 rounded-lg text-sm transition-colors disabled:opacity-50"
            title="Import Vocabulary Progress"
          >
            <Upload className="w-4 h-4" /> <span className="hidden sm:inline">Import Vocab</span>
          </button>
          <button 
          ...
          >
            <Download className="w-4 h-4" /> <span className="hidden sm:inline">Export Vocab</span>
          </button>
        </div>
      </header>
            ...
            <div className="p-4 border-b border-slate-200 bg-slate-50 rounded-t-xl flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                ...
                <button 
                  ...
                  disabled={!currentLang || !inputText}
                ...
                <div className="flex flex-col gap-2">
                  {filteredLibrary.map(book => (
                    <div 
                      ...
                      className={`p-3 bg-white border ${activeStoryId === book.id ? 'border-indigo-500 shadow-md bg-indigo-50' : 'border-slate-200 hover:border-indigo-300 hover:shadow-sm'} rounded-lg cursor-pointer transition-all group relative`}
                    >
            ...
            <>
                    ...
                    <>
                      {isPlayingFullStory ? (
                        <button 
                          ...
                          className="flex items-center justify-center gap-2 bg-red-50 text-red-600 border border-red-200 hover:bg-red-100 px-4 py-2 rounded-lg shadow-sm font-medium transition-colors w-40"
                        >
                          {isStoryAudioLoading ? (
                      ...
                      >
                        ...
                        <span className="hidden sm:inline">{showFullTranslationToggle ? 'Hide Translation' : 'Translate Chapter'}</span>
                      </button>
                    </>
                  )}
              ...
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 md:p-8 min-h-[400px] flex flex-col relative" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="flex flex-col gap-3 w-full md:w-auto">
                      ...
                      <div className="flex flex-col gap-2 w-full">
                        ...
                        <button 
                          ...
                          className="w-full flex items-center justify-center gap-2 bg-white text-blue-700 border border-blue-300 hover:bg-blue-50 px-5 py-2.5 rounded-lg font-medium transition-colors"
                ...
                {!isReading ? (
                  ...
                  <div 
                    className="text-xl md:text-2xl leading-relaxed md:leading-loose flex-1 pb-10"
                ...
                {isReading && activeChapters.length > 0 && activeChapterIndex === activeChapters.length - 1 && (
                  <div className="mt-8 p-5 bg-indigo-50/50 border border-indigo-100 rounded-xl flex flex-col gap-3">
                    <h4 className="font-bold text-indigo-900 flex items-center gap-2 text-sm uppercase tracking-wide mb-1">
      ...
      {/* CHAT / ROLEPLAY MODAL */}
      {isChatOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 sm:p-6 z-[100]" onClick={() => setIsChatOpen(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl h-[85vh] max-h-[800px] flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
             {/* Header */}
             <div className="bg-indigo-600 text-white px-5 py-4 flex justify-between items-center">
               <div>
                 <h2 className="font-bold flex items-center gap-2 text-lg">
                   <MessageCircle className="w-5 h-5"/> {currentLang?.name} Roleplay Partner
                 </h2>
                 <p className="text-indigo-200 text-sm mt-0.5">CEFR Level: {storyLevel}</p>
               </div>
               <div className="flex items-center gap-3">
                 <button 
                   onClick={() => {
             ...
             {/* Messages */}
             <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-slate-50 flex flex-col gap-4" ref={chatScrollRef}>
                      ...
                      <div className={`px-4 py-3 text-[1.05rem] leading-relaxed shadow-sm relative group ${
                         ...
                         {msg.role === 'model' && (
                           <button 
                             ...
                             className="absolute -right-10 top-2 p-2 text-slate-400 hover:text-blue-600 transition-colors opacity-0 group-hover:opacity-100"
      ...
      {/* Action Bar (Fixed at bottom when words are selected, story translation is open, or grammar info is open) */}
      {(selectedWords.length > 0 || showFullTranslationToggle || infoPopup) && isReading && currentLang && !isChatOpen && (
        <div 
        ...
        >
          <div className="max-w-4xl mx-auto flex flex-col gap-4">
            ...
            {/* Grammar / Etymology Popup (Now at the bottom) */}
            {infoPopup && (
              ...
              <div className={`pt-3 ${selectedWords.length > 0 ? 'border-t border-slate-100' : ''}`}>
                <div className="flex items-start gap-3 bg-indigo-50/50 p-4 rounded-lg border border-indigo-100 relative">
                  ...
                  <div className="flex-1">
                    <h4 className="text-sm font-bold text-indigo-800 mb-1">Chapter Translation</h4>
                    {isTranslatingFullStory ? (
                       <div className="flex items-center gap-2 text-indigo-500 py-2">
                         <div className="w-4 h-4 border-2 border-indigo-300 border-t-indigo-700 rounded-full animate-spin"></div>
                         <span className="text-sm">Translating chapter...</span>
                       </div>
                    ) : (
