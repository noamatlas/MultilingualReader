...
export default function App() {
  ...
  const parseTeacherResponse = (rawText) => {
     ...
     return { feedback: "", hiddenTarget: "", message: rawText, currentSentence: "", task: "", hint: "", isCorrect: null };
  };
  ...
  const getActivitySystemInstruction = (mode) => {
    ...
    if (mode === 'translation') {
       ...
       3. If the user was CORRECT (or if this is the very first turn), their new valid sentence becomes the new 'currentSentence'. If this is the first turn, invent a natural Seed Sentence in ${currentLang.name}.
  ...
  const handleSaveChatAsBook = () => {
    ...
    const chatText = chatMessages.map(msg => {
      if (activityMode === 'variation_lab') {
         if (msg.role === 'model') {
            let t = `[Base Sentence]: ${msg.currentSentence || msg.message}\n[Task]: ${msg.task || msg.message}`;
