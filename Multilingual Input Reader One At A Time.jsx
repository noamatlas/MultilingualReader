...
import { BookOpen, Edit3, Download, Upload, Volume2, CheckCircle, Clock, X, Globe, AlignLeft, Sparkles, StopCircle, BookA, Languages, Undo2, Search, ArrowDown, GraduationCap, Library, Save, Trash2, BookMarked, ChevronLeft, ChevronRight, Plus, Eye, EyeOff, MessageCircle, Send, Lightbulb, Minimize, Maximize, ChevronFirst, ChevronLast } from 'lucide-react';
...
export default function App() {
  ...
  }, [activeStoryId, activeChapterIndex]);
                ...
                <div className="flex items-center gap-2 ml-auto">
                  {isReading && inputText && (
                    <button
                      ...
                      className={`flex items-center justify-center w-10 h-10 rounded-lg shadow-sm border transition-colors ${isSentenceMode ? 'bg-purple-50 border-purple-200 text-purple-700' : 'bg-white border-slate-300 text-slate-700 hover:bg-slate-100'}`}
                      title="Toggle Focus Mode (One sentence at a time)"
                    >
                    ...
                    {isGeneratingStory ? (
                       ...
                       <div className="flex flex-col items-center justify-center py-10 md:py-20 min-h-[300px] transition-all">
                          ...
                          <div className="flex items-center gap-4 mt-16 bg-slate-50 border border-slate-200 px-6 py-3 rounded-full shadow-sm text-slate-600">
                             <button 
                                onClick={(e) => { e.stopPropagation(); setCurrentSentenceIndex(0); stopAudio(); setShowFullTranslationToggle(false); setSelectedWords([]); }} 
                             ...
                             <button 
                                onClick={(e) => { e.stopPropagation(); setCurrentSentenceIndex(prev => prev - 1); stopAudio(); setShowFullTranslationToggle(false); setSelectedWords([]); }} 
                                disabled={currentSentenceIndex === 0} 
                             ...
                             <button 
                                onClick={(e) => { e.stopPropagation(); setCurrentSentenceIndex(sentenceTokensGroups.length - 1); stopAudio(); setShowFullTranslationToggle(false); setSelectedWords([]); }} 
                                disabled={currentSentenceIndex === sentenceTokensGroups.length - 1} 
                                className="p-2 hover:text-indigo-600 hover:bg-indigo-50 rounded-full transition-colors disabled:opacity-30 disabled:hover:bg-transparent"
                                title="Last Sentence"
                             >
