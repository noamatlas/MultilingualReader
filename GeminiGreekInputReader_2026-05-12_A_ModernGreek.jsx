...
import { BookOpen, Edit3, Download, Upload, Volume2, CheckCircle, Clock, X, Globe, AlignLeft, Sparkles, StopCircle, BookA, Languages, Undo2, Search, ArrowDown, GraduationCap, Library, Save, Trash2, BookMarked, ChevronLeft, ChevronRight, Plus, Eye, EyeOff, MessageCircle, Send, Lightbulb, Minimize, Maximize, ChevronFirst, ChevronLast, LayoutList, Map, Info, Target, Shuffle } from 'lucide-react';
...
export default function App() {
  ...
  const [activityMode, setActivityMode] = useState('free_chat'); // 'free_chat', 'translation', 'cloze', 'scramble'
  ...
  const getActivitySystemInstruction = (mode) => {
    ...
    if (currentLang.code === 'zh-CN' || currentLang.code === 'zh-TW') {
       sysInstruction += `\n6. CRITICAL: For Chinese, include Hanzi AND Pinyin in your 'message' and 'hiddenTarget'.`;
    }
              ...
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col h-[600px] xl:h-[calc(100vh-120px)] overflow-hidden" onClick={(e) => e.stopPropagation()}>
                 ...
                 <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-slate-50 flex flex-col gap-4" ref={chatScrollRef} onClick={() => { setSelectedWords([]); setInfoPopup(null); }}>
                    ...
                    {chatMessages.map((msg, idx) => {
                       // The AI might speak English during Translation drills, so we don't strictly enforce RTL on the AI's prompt there
