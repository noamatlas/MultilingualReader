...
import { BookOpen, Edit3, Download, Upload, Volume2, CheckCircle, Clock, X, Globe, AlignLeft, Sparkles, StopCircle, BookA, Languages, Undo2, Search, ArrowDown, GraduationCap, Library, Save, Trash2, BookMarked, ChevronLeft, ChevronRight, Plus, Eye, EyeOff } from 'lucide-react';
...
export default function App() {
  ...
  const [activeChapters, setActiveChapters] = useState([]); // Array of { text, level, meta, topic, chapterTitle }
  ...
  // Interaction state
  const [selectedWords, setSelectedWords] = useState([]); // { index, token, clean, source }
  ...
  // Sync Input Text with Active Chapters during Edit Mode
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < bodyTokens.length; i++) {
      ...
      if (matchCount === targetWords.length) {
        results.push(currentMatchIndices);
      }
    }
  ...
  const handleGenerateNextChapter = async () => {
    ...
    if (generatedText) {
      ...
      // Clear next chapter inputs so they are fresh for the following generation
      setNextChapterTopic("");
      setNextChapterMeta("");
  ...
  const handleWordClick = (e, token, index, source) => {
    ...
    setSelectedWords(prev => {
      // Cross-zone clicking resets selection
  ...
  return (
    ...
    >
      ...
      {(selectedWords.length > 0 || showFullTranslationToggle || infoPopup) && isReading && currentLang && (
        <div 
