...
// Languages that require Right-to-Left (RTL) text direction
...
export default function App() {
  ...
  // Tabbed Right Panel State
  const [isRightPanelOpen, setIsRightPanelOpen] = useState(true);
  const [rightPanelTab, setRightPanelTab] = useState('vocab'); // 'vocab' or 'plan'
  ...
  const [planChapterMode, setPlanChapterMode] = useState('focus'); // 'focus' or 'plot'
  ...
  useEffect(() => {
    ...
    setHasTTS(true); // Using Gemini TTS for everything now
  }, [currentLang?.code]);
    ...
    if (chatMessages.length === 0) {
      ...
      if (activePlan) {
        if (planChapterMode === 'focus') {
           sysInstruction += `\nCRITICAL LANGUAGE FOCUS: You MUST design your questions to specifically force the user to practice this concept/vocabulary: "${activePlan.title}" (${activePlan.description}).`;
        } else {
           sysInstruction += `\nCRITICAL TOPIC FOCUS: You MUST center the conversation around discussing and exploring this topic: "${activePlan.title}" (${activePlan.description}).`;
        }
      }
