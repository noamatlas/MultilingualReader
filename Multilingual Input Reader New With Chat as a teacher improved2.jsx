...
import { BookOpen, Edit3, Download, Upload, Volume2, CheckCircle, Clock, X, Globe, AlignLeft, Sparkles, StopCircle, BookA, Languages, Undo2, Search, ArrowDown, GraduationCap, Library, Save, Trash2, BookMarked, ChevronLeft, ChevronRight, Plus, Eye, EyeOff, MessageCircle, Send, Lightbulb } from 'lucide-react';
...
export default function App() {
  ...
  const [chatMessages, setChatMessages] = useState([]); // Array of { role: 'user'|'model', text: string, feedback?: string, hiddenTarget?: string, isHintOpen?: boolean }
  ...
  // Tokenize chat messages - using only the core 'text' field which contains the target language output
  ...
  // Dedicated Chat API that strictly enforces JSON to manage the teaching state
  const callGeminiChatAPI = async (history, sysInstruction) => {
    const apiKey = ""; 
    const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${apiKey}`;
    
    // Reconstruct history to ensure the AI remembers its own hidden targets
  ...
  const parseTeacherResponse = (rawText) => {
     try {
       // Attempt to extract JSON from the text block
     ...
     // Fallback if AI forgets formatting
  ...
  const handleOpenChat = async () => {
    ...
    if (chatMessages.length === 0) {
      ...
      const sysInstruction = `You are a structured language teacher using the Pimsleur/Guided active recall method.
