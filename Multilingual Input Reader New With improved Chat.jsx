...
export default function App() {
  ...
  // Keep a ref of all tokens for the translation context engine to fetch from
  ...
  useEffect(() => {
    let isMounted = true;
    if (isReading && inputText && currentLang && hasTTS && !isChatOpen) {
    ...
    return () => { isMounted = false; };
  }, [isReading, inputText, currentLang, hasTTS, isChatOpen]);
  ...
  useEffect(() => {
    ...
    if (isChatOpen) {
       chatTokensArray.forEach(tokens => {
          allTextWords.push(...tokens.filter(t => t.isWordLike).map(t => cleanWord(t.text)));
       });
    }
    ...
    fetchRomanizations();
    return () => { isMounted = false; };
  }, [bodyTokens, titleTokens, chapterTitleTokens, chatTokensArray, isChatOpen, vocab, isReading, currentLang]);
  ...
  const handleSaveChatAsBook = () => {
    ...
    const chatText = chatMessages.map(msg => 
      `${msg.role === 'user' ? 'Me' : currentLang.name + ' Partner'}:\n${msg.text}`
    ).join('\n\n');

    let resolvedTitle = `Chat: ${storyTopic.trim() || 'Roleplay'}`;
  ...
  return (
    ...
    >
                       ...
                       <div key={idx} className={`flex flex-col max-w-[85%] ${msg.role === 'user' ? 'self-end items-end' : 'self-start items-start'}`}>
                          <div className={`px-4 py-3 text-[1.05rem] leading-relaxed shadow-sm relative group ${
                             ...
                             <button 
                               ...
                               className={`absolute ${msg.role === 'user' ? '-left-10' : '-right-10'} top-2 p-2 text-slate-400 hover:text-blue-600 transition-colors opacity-0 group-hover:opacity-100`}
      ...
      {(selectedWords.length > 0 || showFullTranslationToggle || infoPopup) && currentLang && (isReading || isChatOpen) && (
        <div 
